...
test("导航那块高度是量出来的，不是拍脑袋写死的", () => {
  ...
  // 线上真机形状的验证里量到的：收藏徽章从无到有，导航就从 84px 变成
  // 87px，而 --nav-h 还停在启动时那个数——首页和收藏页最下面一行被压掉
  // 2–3px。挂在 load/resize/fonts 上不够：导航的高度会因为它自己的内容
  // 变化，而那些事件一个都不会触发。
  assert.match(product, /new ResizeObserver\(/,
    "只盯着窗口事件：导航因为徽章出现而变高时，留白不会跟着变");
  // 但观察者本身在后台标签页里一次都不触发，也就没法在自动化里验证。
  // 已知的那个原因——徽章——必须有一条量得到、验得了的直接路径。
  const badge = product.match(/function updateNavBadge\(\)\s*\{[\s\S]*?\n\}/);
  assert.ok(badge, "找不到刷新徽章的地方");
  assert.match(badge[0], /syncNavHeight\(\)/,
    "徽章变化之后没有重新量导航高度——只靠一个验不了的观察者兜底");
});
