...
step("L0 · storage owns every localStorage read/write (test/storage.test.mjs)", () => run("node", ["test/storage.test.mjs"]));
step("L0 · audio controller owns play/pause (test/audio-controller.test.mjs)", () => run("node", ["test/audio-controller.test.mjs"]));
...
step("L0 · what a saved row is allowed to be (test/saved-row-shape.test.mjs)", () => run("node", ["test/saved-row-shape.test.mjs"]));
step("L0 · reveal + grading buttons (test/reveal-and-buttons.test.mjs)", () => run("node", ["test/reveal-and-buttons.test.mjs"]));
...
step("L0 · tabs reachable from every screen (test/nav-everywhere.test.mjs)", () => run("node", ["test/nav-everywhere.test.mjs"]));
...
step("L0 · push: service worker (test/sw-push.test.mjs)", () => run("node", ["test/sw-push.test.mjs"]));
...
step("L0 · reminder: scheduled clock (test/reminder-cron.test.mjs)", () => run("node", ["test/reminder-cron.test.mjs"]));
